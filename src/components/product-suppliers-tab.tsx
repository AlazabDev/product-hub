    ...
    queryFn: async () => {
      const [{ data: pricesRows }, { data: invRows }] = await Promise.all([
        supabase
          .from("prices")
          .select(
            "supplier_id, selling_price, purchase_price, currency, suppliers(id, name, contact_name, email, phone, supplier_tier)",
          )
          .eq("product_id", productId)
          .not("supplier_id", "is", null),
        supabase
          .from("supplier_inventory")
          .select("*, suppliers(id, name, contact_name, email, phone, supplier_tier)")
    ...
    return (
      <Card className="p-12 text-center surface-elevated border-0">
        ...
        <p className="text-xs text-muted-foreground/70 mt-1">
          أضف سعر مرتبط بمورد في تاب التسعير ليظهر هنا.
        </p>
      </Card>
    );
  }
      ...
      {rows.map((r: any) => (
        <Card key={r.supplier.id} className="p-4 surface-elevated border-0">
          <div className="flex items-start justify-between">
            <div>
              ...
              {r.supplier.contact_name && (
                <p className="text-xs text-muted-foreground mt-0.5">{r.supplier.contact_name}</p>
              )}
            </div>
            <span className="text-[10px] bg-accent/15 text-accent rounded px-2 py-0.5">
              {r.supplier.supplier_tier === "first_tier"
                ? "درجة أولى"
                : r.supplier.supplier_tier === "backup"
                  ? "احتياطي"
                  : "درجة ثانية"}
            </span>
          </div>
          <div className="mt-2 space-y-1 text-xs">
            ...
            {r.supplier.phone && (
              <div className="flex items-center gap-1.5 text-muted-foreground num" dir="ltr">
          ...
          {r.price && (
            <div className="mt-3 pt-3 border-t grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <div className="num font-semibold" dir="ltr">
                  {fmt(r.price.selling)} {r.price.currency}
                </div>
              </div>
              <div>
                <div className="text-muted-foreground">سعر الشراء</div>
                <div className="num font-semibold" dir="ltr">
                  {fmt(r.price.purchase)} {r.price.currency}
                </div>
              </div>
            </div>
          )}
          {r.inv && (
            <div className="mt-2 pt-2 border-t text-xs">
              ...
              <div className="num font-semibold mt-0.5" dir="ltr">
                {r.inv.available_quantity ?? "—"} · {r.inv.availability_status ?? "غير محدد"}
              </div>
            </div>
          )}
        </Card>
