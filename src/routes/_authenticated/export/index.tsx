...
const TARGETS = [
  {
    ...
    format: "csv",
    desc: "تصدير المنتجات والأسعار بتنسيق دفترة",
  },
  {
    id: "erpnext",
    label: "ERPNext",
    icon: Database,
    format: "csv",
    desc: "تنسيق ERPNext Item Master",
  },
  {
    id: "ai_knowledge",
    ...
    icon: Sparkles,
    format: "jsonl",
    desc: "JSONL لتغذية Azure AI Search/Vector DB",
  },
  {
    id: "json_full",
    label: "JSON كامل",
    icon: FileJson,
    format: "json",
    desc: "كل بيانات المنتجات مع الصور والأسعار",
  },
  {
    id: "csv_basic",
    label: "CSV بسيط",
    icon: FileSpreadsheet,
    format: "csv",
    desc: "AZ Code, الاسم, الوصف, السعر",
  },
];
  ...
  const run = async (target: (typeof TARGETS)[number]) => {
    ...
    try {
      let query = supabase
        .from("products")
        .select(
          "az_code, egs_code, name_ar, name_en, short_description_ar, description_ar, gpc_brick_title, tags, status, updated_at",
        )
        .limit(5000);
      ...
      const { data: prices } = await supabase
        .from("prices")
        .select("product_id, selling_price, purchase_price, currency")
        .eq("status", "approved");
      const priceMap = new Map<
        string,
        { selling: number | null; purchase: number | null; currency: string }
      >();
      prices?.forEach((p) => {
      ...
      if (target.id === "ai_knowledge") {
        content = products
          .map((p) =>
            JSON.stringify({
              ...
              category: p.gpc_brick_title,
              tags: p.tags ?? [],
            }),
          )
          .join("\n");
        filename = `ai-knowledge-${Date.now()}.jsonl`;
        mime = "application/x-ndjson";
      } else if (target.id === "json_full") {
        const enriched = products.map((p) => ({ ...p }));
        content = JSON.stringify(
          { exported_at: new Date().toISOString(), count: enriched.length, products: enriched },
          null,
          2,
        );
        filename = `products-full-${Date.now()}.json`;
        ...
        content = toCSV(rows);
        filename = `daftra-export-${Date.now()}.csv`;
        mime = "text/csv";
      } else if (target.id === "erpnext") {
        const rows = products.map((p) => ({
          item_code: p.az_code,
          item_name: p.name_ar,
          item_group: p.gpc_brick_title ?? "All Item Groups",
          stock_uom: "Nos",
          description: p.description_ar ?? p.short_description_ar ?? "",
          is_sales_item: 1,
          is_purchase_item: 1,
        }));
        content = toCSV(rows);
        filename = `erpnext-items-${Date.now()}.csv`;
        mime = "text/csv";
      } else {
        const rows = products.map((p) => {
    ...
    <div className="container py-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          تصدير البيانات بتنسيقات Daftra / ERPNext / AI Knowledge / JSON / CSV
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">سجل التصديرات</CardTitle>
        </CardHeader>
