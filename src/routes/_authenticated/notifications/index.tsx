    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          مركز كل الإشعارات وسجل النشاط اللحظي على المنصة
        </p>
      </div>
      ...
      <Tabs defaultValue="notifications">
        <TabsList>
          <TabsTrigger value="notifications" className="gap-1.5">
            <Bell className="size-3.5" /> الإشعارات
          </TabsTrigger>
          <TabsTrigger value="activity" className="gap-1.5">
            <Activity className="size-3.5" /> سجل النشاط
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="notifications" className="mt-4 space-y-2">
          {nLoading ? (
            ...
            notifications.map((n) => (
              <Card
                key={n.id}
                className={`p-4 surface-elevated border-0 ${!n.is_read ? "border-r-2 border-r-accent" : ""}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {n.link ? (
                      <Link to={n.link} className="font-semibold hover:text-accent">
                        {n.title}
                      </Link>
                    ) : (
        ...
        <TabsContent value="activity" className="mt-4 space-y-2">
          {aLoading ? (
            ...
            activity.map((a) => (
              <Card
                key={a.id}
                className="p-3 surface-elevated border-0 flex items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-muted-foreground num truncate" dir="ltr">
                      {a.entity_id}
                    </div>
                  </div>
                </div>
